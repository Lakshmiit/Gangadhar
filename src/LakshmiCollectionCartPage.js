...
const LakshmiCollectionCartPage = () => {
    ...
    const fetchCart = async () => {
      try {
        ...
        const response = await fetch(
          `https://localhost:7091/api/LakshmiCollection/GetLakshmiCollectionDetails/${uploadedId}`,
        );
            ...
            if (productImageFilename) {
              try {
                const imgRes = await fetch(
                  `https://localhost:7091/api/FileUpload/download?generatedfilename=${productImageFilename}`,
                );
  ...
  const handleUpdateCollection = async (event) => {
    ...
    try {
      const response = await fetch(
        `https://localhost:7091/api/LakshmiCollection/UpdateLakshmiCollectionDetails/${uploadedId}`,
        {
          method: "PUT",
