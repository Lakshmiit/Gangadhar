...
// // Fallback vendorId used only if no vendor is currently selected in CartStorage.
// const DEFAULT_VENDOR_ID = "a1acccca-a740-40c0-9b82-38778e58a4fa";
...
//   const [zoomImage, setZoomImage] = useState("");
//   const [grandSummary, setGrandSummary] = useState({ items: 0, total: 0 });
//   // vendorStockMap: { [productId]: stockQuantity } sourced from GetVendorProductsvalues
...
//   // Fetches the vendor's product list (GetVendorProductsvalues) and flattens
//   // it into a { [productId]: stockQuantity } map. This replaces the old
//   // GetGroceryItemsByProductName-by-name lookup with a productId based one.
//   const fetchVendorStockMap = useCallback(async () => {
//     const vendorId = selectedVendorId || DEFAULT_VENDOR_ID;
//     try {
//       const res = await fetch(
//         `https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/VendorUploadProducts/GetVendorProductsvalues?vendorId=${vendorId}`,
//       );
//       if (!res.ok) return {};
//       const data = await res.json();
//       const vendorArr = Array.isArray(data) ? data : [data];
//       const map = {};
//       vendorArr.forEach((vendor) => {
//         (vendor.categorie || vendor.categories || []).forEach((cat) => {
//           (cat.products || []).forEach((p) => {
//             const pid = p.productIds ?? p.productId ?? p.id;
//             if (!pid) return;
//             // quantity from the vendor payload represents current stock.
//             map[pid] = Number(p.quantity ?? 0);
//           });
//         });
//       });
//       return map;
//     } catch (err) {
//       console.error("Vendor stock fetch error:", err);
//       return {};
//     }
//   }, [selectedVendorId]);

//   const getDynamicStock = useCallback(
//     (productId) => {
//       if (!productId) return Infinity;
//       const val = vendorStockMap[productId];
//       return val !== undefined ? Number(val) : Infinity;
//     },
//     [vendorStockMap],
//   );

//   // Populate vendorStockMap (and its ref, for synchronous access) once on
//   // mount and whenever the selected vendor changes.
//   const vendorStockMapRef = useRef({});
//   useEffect(() => {
//     let cancelled = false;
//     (async () => {
//       const map = await fetchVendorStockMap();
//       if (cancelled) return;
//       vendorStockMapRef.current = map;
...
//   const getDynamicStockRef = (productId) => {
//     if (!productId) return Infinity;
//     const val = vendorStockMapRef.current[productId];
//     return val !== undefined ? Number(val) : Infinity;
//   };
...
//           const imageUrl = getAzureImageUrl(persisted);
//           const rawQty = Number(p.qty);
//           const productId = p.productId ?? p.id ?? idx;
//           const limit = getDynamicStockRef(productId);
//           const stock = Number(p.stockLeft || Infinity);
//           return {
//             id: `${cat.categoryName}-${productId}`,
//             productId,
//             name: p.productName ?? p.name ?? "",
//             category: cat.categoryName,
//             qty: Math.min(rawQty, limit, stock),
...
//         .map((it) => {
//           if (it.id !== rowId) return it;
//           const limitMax = getDynamicStock(it.productId);
//           const maxAllowed = Math.min(latestStock, limitMax);
//           const proposedQty = Number(it.qty || 0) + delta;
//           if (latestStock <= 0) {
//             return {
...
//   // Resolves the latest available stock for a cart item.
//   // - Handyman categories keep using GetProductsByProductName (by name).
//   // - Everything else is now resolved via GetVendorProductsvalues, matched
//   //   by productId: if the productId's quantity is present it is returned
//   //   as the stock, otherwise the item is treated as out of stock (0).
//   const fetchLatestStock = useCallback(
//     async (productId, category = "", productName = "") => {
//       const isHandymanCategory = HANDYMAN_CATEGORIES.includes(
//         String(category || "")
//           .trim()
//           .toLowerCase(),
...
//       try {
//         const vendorMap = await fetchVendorStockMap();
//         const stock = vendorMap[productId];
//         return stock !== undefined ? Number(stock) : 0;
//       } catch (err) {
//         console.error("Vendor stock lookup error:", err);
//         return Infinity;
//       }
//     },
//     [fetchLatestStockForHandyman, fetchVendorStockMap],
//   );
...
//     const updated = currentItems
//       .map((item) => {
//         const latest = stockResults.find((x) => x.productId === item.productId);

//         if (!latest) return item;

//         const limit = getDynamicStock(item.productId);
...
//     writeBackToStorage(updated);
//     setGrandSummary(computeTotals(updated));
//   }, [fetchLatestStock, getDynamicStock, writeBackToStorage]);
...
//         const updated = prev
//           .map((item) => {
//             const latest = checks.find((x) => x.productId === item.productId);
...
//             comboData.items.map(async (item) => ({
//               productName: item.productName,
//               // Combo items are stored without a productId, so we fall back
//               // to whatever fetchLatestStock resolves for a missing id
//               // (handyman lookup by name, otherwise treated as available).
//               stockLeft: await fetchLatestStock(
//                 item.productId,
//                 item.category,
//                 item.productName,
...
const GroceryCartPage = () => {
  ...
  const [zoomImage, setZoomImage] = useState("");
  const [grandSummary, setGrandSummary] = useState({ items: 0, total: 0 });
  // vendorStockMap: { [productId]: { stock, limit, maxAllowed } } sourced from
  // GetVendorProductsvalues. `maxAllowed` is the effective cap a customer can
  // add to the cart, i.e. min(stock, limit) when a per-order limit is set.
  const [vendorStockMap, setVendorStockMap] = useState({});
  const [addresses, setAddresses] = useState([]);
  const [fullName, setFullName] = useState("");
  const [isNewUser, setIsNewUser] = useState(true);
  ...
  // Fetches the vendor's product list (GetVendorProductsvalues) and flattens
  // it into a { [productId]: { stock, limit, maxAllowed } } map. This replaces
  // the old GetGroceryItemsByProductName-by-name lookup with a productId
  // based one, and now also respects the per-product "limit" field returned
  // by the API (max quantity a customer may add for that product), not just
  // raw "quantity" (available stock).
  //
  // Rule: if `limit` > 0, the effective cap is min(quantity, limit).
  //       if `limit` is 0/absent, there is no per-order cap, so the cap is
  //       just the available `quantity`.
  const fetchVendorStockMap = useCallback(async () => {
    const vendorId = selectedVendorId || DEFAULT_VENDOR_ID;
    try {
      const res = await fetch(
      ...
      vendorArr.forEach((vendor) => {
        (vendor.categorie || vendor.categories || []).forEach((cat) => {
          (cat.products || []).forEach((p) => {
            ...
            const maxAllowed = limit > 0 ? Math.min(stock, limit) : stock;
            map[pid] = { stock, limit, maxAllowed };
          });
        });
      });
      return map;
  ...
  // Returns the effective cap (stock capped by per-order limit, if any) for
  // a productId. Falls back to Infinity if we have no data for it.
  const getDynamicStock = useCallback(
    (productId) => {
      if (!productId) return Infinity;
      const entry = vendorStockMap[productId];
      return entry !== undefined ? entry.maxAllowed : Infinity;
    },
    [vendorStockMap],
  );
  ...
  const getDynamicStockRef = (productId) => {
    if (!productId) return Infinity;
    const entry = vendorStockMapRef.current[productId];
    return entry !== undefined ? entry.maxAllowed : Infinity;
  };
    ...
    const items = saved.flatMap((cat) =>
      (cat.products || [])
        ...
        .map((p, idx) => {
          ...
          const rawQty = Number(p.qty);
          const productId = p.productId ?? p.id ?? idx;
          // effective cap = min(persisted stockLeft, live stock, per-order limit)
          const limit = getDynamicStockRef(productId);
          const stock = Number(p.stockLeft || Infinity);
          return {
            id: `${cat.categoryName}-${productId}`,
  ...
  const handleQtyChange = async (rowId, delta) => {
    ...
    setCartItems((prev) => {
      const updated = prev
        .map((it) => {
          if (it.id !== rowId) return it;
          // limitMax already accounts for the per-order "limit" field
          // (see getDynamicStock / fetchVendorStockMap above)
          const limitMax = getDynamicStock(it.productId);
          const maxAllowed = Math.min(latestStock, limitMax);
          const proposedQty = Number(it.qty || 0) + delta;
          if (latestStock <= 0) {
  ...
  // Resolves the latest available (limit-capped) stock for a cart item.
  // - Handyman categories keep using GetProductsByProductName (by name).
  // - Everything else is resolved via GetVendorProductsvalues, matched by
  //   productId: if the productId's entry is present, the effective cap
  //   (min(quantity, limit) when limit > 0) is returned as the stock,
  //   otherwise the item is treated as out of stock (0).
  const fetchLatestStock = useCallback(
    async (productId, category = "", productName = "") => {
      const isHandymanCategory = HANDYMAN_CATEGORIES.includes(
        String(category || "")
      ...
      try {
        const vendorMap = await fetchVendorStockMap();
        const entry = vendorMap[productId];
        return entry !== undefined ? entry.maxAllowed : 0;
      } catch (err) {
        console.error("Vendor stock lookup error:", err);
        return Infinity;
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {cartItems.map((item) => {
          const isOutOfStock = item.outOfStock || item.stockLeft <= 0;
          // effective per-order cap (min(stock, limit)) for disabling "+"
          const maxAllowedForItem = getDynamicStock(item.productId);
          const capForButton = Math.min(item.stockLeft, maxAllowedForItem);
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {/* Show the per-order limit note when it is the binding constraint */}
                  {!isOutOfStock &&
                    Number.isFinite(maxAllowedForItem) &&
                    maxAllowedForItem < item.stockLeft && (
                      <div style={{ fontSize: "10px", color: "#d32f2f" }}>
                        Max {maxAllowedForItem} per order
                      </div>
                    )}
                </div>
              </div>
              ...
              {isOutOfStock ? (
                ...
                >
                  ...
                  <IconButton
                    ...
                    onClick={() => handleQtyChange(item.id, 1)}
                    style={{ color: "white", padding: "2px" }}
                    disabled={item.qty >= capForButton}
                  >
                    <AddIcon fontSize="small" />
                  </IconButton>
                </div>
