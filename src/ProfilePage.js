...
// //   // Super admin's own session — checked client-side, same as SuperAdminGuard.
// //   // Kept in state (not just a plain call) so the icon/label swap below
// //   // re-renders immediately after login/logout, without needing a reload.
// //   const [isSuperAdmin, setIsSuperAdmin] = useState(() =>
// //     isSuperAdminAuthenticated(),
// //   );
// //   useEffect(() => {
// //     const syncSuperAdmin = () => setIsSuperAdmin(isSuperAdminAuthenticated());
// //     syncSuperAdmin();
// //     window.addEventListener("storage", syncSuperAdmin);
// //     window.addEventListener("focus", syncSuperAdmin);
// //     return () => {
// //       window.removeEventListener("storage", syncSuperAdmin);
// //       window.removeEventListener("focus", syncSuperAdmin);
// //     };
// //   }, []);

// //   const handleVendorPortal = () => {
// //     // Logged-in super admin: the "Vendor Portal" tile becomes a direct
// //     // shortcut into the super admin vendors console — skip the vendor
// //     // login flow entirely.
// //     if (isSuperAdmin) {
...
// //                             })
// //                           )}
// //                           {!isSuperAdmin && renderVendorOrderBell()}
// //                         </div>
// //                         <small
// //                           style={{
// //                             fontSize: "12px",
...
// //                                 style={{ fontSize: "40px" }}
// //                               >
// //                                 {menu.MenuTitle === "Vendor Portal" && isSuperAdmin ? (
// //                                   <span
...
// //                                   menu.MenuIcon
// //                                 )}
// //                                 {menu.MenuTitle === "Vendor Portal" && !isSuperAdmin && renderVendorOrderBell()}
...
//       (v) => v.vendorId === selectedMartTab,
//     );
//     // Default to the top-ranked category so the pre-selected tile matches
//     // whatever renders first in the (rank-sorted) tile list below.
//     const topCategory = [...(vendor?.categories || [])].sort(
...
//                       );
//                       if (!vendor) return null;
//                       // Sort this vendor's categories by rank (lower rank
//                       // shows first). Categories without a rank field fall
//                       // back to the end, in their original order.
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const effectivePincode =
      String(zipCode || pinCode || DEFAULT_PINCODE).trim() || DEFAULT_PINCODE;

    let cancelled = false;

    (async () => {
      try {
        const vendors = await getVendorsByPincode(effectivePincode);

        if (cancelled) return;

        let approvedVendors = vendors.filter((v) => v.status === "Approved");
        ...
        setApprovedVendorListJson(approvedVendors);
  ...
  useEffect(() => {
        ...
        if (vendorKnownOrderIdsRef.current) {
          ...
          if (previousStatusById) {
            const justDelivered = list.filter((o) => {
              ...
              const wasDelivered =
                String(prevStatus || "").toLowerCase() === "delivered";
              return prevStatus !== undefined && nowDelivered && !wasDelivered;
            });
            if (justDelivered.length) {
              setTimeout(() => {
                justDelivered.forEach((order) =>
                  speakOrderDeliveredAlert(order),
                );
              }, 600);
            }
          }
    ...
    const interval = setInterval(
      pollVendorOrders,
      VENDOR_ORDERS_POLL_INTERVAL_MS,
    );
    return () => {
      cancelled = true;
      clearInterval(interval);
          ...
          if (arrivedOrders.length) {
            ...
            setTimeout(() => {
              arrivedOrders.forEach((order) =>
                speakNewDeliveryAssignmentAlert(order),
              );
            }, 600);
          }
        }
                ...
                <div className="shadow-lg p-2 rounded-5 text-center bg-transparent border-0">
                  ...
                  {selectedMartTab !== "Lakshmi Mart" &&
                    (() => {
                      ...
                      const unbeatableCategory =
                        rawCategories.find(isUnbeatableCategory);
